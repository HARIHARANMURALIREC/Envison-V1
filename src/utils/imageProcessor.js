class ImageProcessor {
      ...
      for (let x = halfKernel; x < width - halfKernel; x++) {
        ...
        for (let ky = -halfKernel; ky <= halfKernel; ky++) {
          for (let kx = -halfKernel; kx <= halfKernel; kx++) {
            ...
            values.push(data[kIdx], data[kIdx + 1], data[kIdx + 2]);
          }
        }
        ...
        newData[idx] = values[medianIdx];     // R
        newData[idx + 1] = values[medianIdx]; // G
        newData[idx + 2] = values[medianIdx]; // B
      }
    }
  ...
  histogramEqualization() {
    ...
    for (let i = 0; i < 256; i++) {
      cdf[i] = ((cdf[i] - cdfMin) / (grayData.length / 4 - cdfMin)) * 255;
    }
    ...
    try {
      ...
      for (let y = halfKernel; y < height - halfKernel; y++) {
        for (let x = halfKernel; x < width - halfKernel; x++) {
          ...
          newData[idx] = Math.max(0, Math.min(255, r));     // R
          newData[idx + 1] = Math.max(0, Math.min(255, g)); // G
          newData[idx + 2] = Math.max(0, Math.min(255, b)); // B
        }
      }
